...
module.exports = {
    ...
    // Adiciona os anúncios
    addAction: async (req, res) => {
        let { title, price, priceneg, desc, cat, token  } = req.body;
        const user = await User.findOne({token}).exec();

        // Verifica se o titulo e a categoria foram preenchidos
        if(!title || !cat) {
            res.json({error: "titulo e/ou categoria nao foram prenchidos"});
            return;
        }
        ...
        if(price) { // Transforma o preço(que é mandado como string) em number(Ex.:'R$8.000,35' = 8000.35)
        ...
        // Adiçaõ de imagem com listagem de imagens
        if(req.files &&  req.files.img) {
            if(['images/jpeg', 'image/jpg', 'image/png'].includes(req.files.img.mimetype)){
                ...
                for(let i = 0; i < req.files.img.length; i++) {
                    if(['images/jpeg', 'image/jpg', 'image/png'].includes(req.files.img[i].mimetype)){
    ...
    getList: async (req, res) => {
        ...
        // Ordena os anúncios por data de postagem(preferencial a ordem)
        ...
        for(let i in adsData) {
            ...
            if(defaultImg) {
                ...
                image = `${process.env.BASE}/media/default`;
            }
    ...
    // Pega um anúncio específico e anúncios relacionados
